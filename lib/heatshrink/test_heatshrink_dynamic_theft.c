...
static void ICACHE_FLASH_ATTR *rbuf_alloc_cb(struct theft *t, theft_hash seed, void *env) {
...
static void ICACHE_FLASH_ATTR rbuf_free_cb(void *instance, void *env) {
    free(instance);
    (void)env;
}

static uint64_t ICACHE_FLASH_ATTR rbuf_hash_cb(void *instance, void *env) {
...
static void ICACHE_FLASH_ATTR *copy_rbuf_subset(rbuf *cur, size_t new_sz, size_t byte_offset) {
    if (new_sz == 0) { return THEFT_DEAD_END; }
    rbuf *nr = malloc(sizeof(rbuf) + new_sz);
    if (nr == NULL) { return THEFT_ERROR; }
    nr->size = new_sz;
    os_memcpy(nr->buf, &cur->buf[byte_offset], new_sz);
    /* printf("%zu -> %zu\n", cur->size, new_sz); */
    return nr;
}
...
static void ICACHE_FLASH_ATTR *copy_rbuf_percent(rbuf *cur, float portion, float offset) {
...
static void ICACHE_FLASH_ATTR *rbuf_shrink_cb(void *instance, uint32_t tactic, void *env) {
...
static void ICACHE_FLASH_ATTR rbuf_print_cb(FILE *f, void *instance, void *env) {
    rbuf *r = (rbuf *)instance;
    (void)env;
    fprintf(f, "buf[%zd]:\n    ", r->size);
    uint8_t bytes = 0;
...
ICACHE_FLASH_ATTR progress_cb(struct theft_trial_info *info, void *env) {
    test_env *te = (test_env *)env;
...
static theft_trial_res ICACHE_FLASH_ATTR prop_should_not_get_stuck(void *input) {
...
static bool ICACHE_FLASH_ATTR get_time_seed(theft_seed *seed)
{
    struct timeval tv;
...
TEST ICACHE_FLASH_ATTR decoder_fuzzing_should_not_detect_stuck_state(void) {
...
static theft_trial_res ICACHE_FLASH_ATTR prop_encoded_and_decoded_data_should_match(void *input) {
...
TEST ICACHE_FLASH_ATTR  encoded_and_decoded_data_should_match(void) {
...
static size_t ICACHE_FLASH_ATTR ceil_nine_eighths(size_t sz) {
    return sz + sz/8 + (sz & 0x07 ? 1 : 0);
}

static theft_trial_res
ICACHE_FLASH_ATTR prop_encoding_data_should_never_increase_it_by_more_than_an_eighth_at_worst(void *input) {
...
TEST ICACHE_FLASH_ATTR encoding_data_should_never_increase_it_by_more_than_an_eighth_at_worst(void) {
    test_env env = { .limit = 1 << 11 };
...
static theft_trial_res
ICACHE_FLASH_ATTR prop_encoder_should_always_make_progress(void *instance) {
    uint8_t output[64 * 1024];
...
TEST ICACHE_FLASH_ATTR encoder_should_always_make_progress(void) {
...
static theft_trial_res
ICACHE_FLASH_ATTR prop_decoder_should_always_make_progress(void *instance) {
    uint8_t output[64 * 1024];
...
TEST ICACHE_FLASH_ATTR decoder_should_always_make_progress(void) {
    test_env env = { .limit = 1 << 15 };
