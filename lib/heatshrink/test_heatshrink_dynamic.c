...
static void ICACHE_FLASH_ATTR dump_buf(char *name, uint8_t *buf, uint16_t count) {
...
TEST ICACHE_FLASH_ATTR encoder_alloc_should_reject_invalid_arguments(void) {
...
TEST ICACHE_FLASH_ATTR encoder_sink_should_reject_nulls(void) {
...
TEST ICACHE_FLASH_ATTR encoder_poll_should_reject_nulls(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 7);
    uint8_t output[256];
...
TEST ICACHE_FLASH_ATTR encoder_finish_should_reject_nulls(void) {
    ASSERT_EQ(HSER_FINISH_ERROR_NULL, heatshrink_encoder_finish(NULL));
    PASS();
}

TEST ICACHE_FLASH_ATTR encoder_sink_should_accept_input_when_it_will_fit(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 7);
    ASSERT(hse);
    uint8_t input[256];
...
TEST ICACHE_FLASH_ATTR encoder_sink_should_accept_partial_input_when_some_will_fit(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 7);
    ASSERT(hse);
    uint8_t input[512];
    size_t bytes_copied = 0;
...
TEST ICACHE_FLASH_ATTR encoder_poll_should_indicate_when_no_input_is_provided(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 7);
    uint8_t output[512];
    size_t output_size = 0;
...
TEST ICACHE_FLASH_ATTR encoder_should_emit_data_without_repetitions_as_literal_sequence(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 7);
    ASSERT(hse);
    uint8_t input[5];
    uint8_t output[1024];
...
TEST ICACHE_FLASH_ATTR encoder_should_emit_series_of_same_byte_as_literal_then_backref(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 7);
    ASSERT(hse);
    uint8_t input[5];
    uint8_t output[1024];
...
TEST ICACHE_FLASH_ATTR encoder_poll_should_detect_repeated_substring(void) {
...
TEST ICACHE_FLASH_ATTR encoder_poll_should_detect_repeated_substring_and_preserve_trailing_literal(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 3);
...
TEST ICACHE_FLASH_ATTR decoder_alloc_should_reject_excessively_small_window(void) {
    ASSERT_EQ(NULL, heatshrink_decoder_alloc(256,
            HEATSHRINK_MIN_WINDOW_BITS - 1, 4));
    PASS();
}

TEST ICACHE_FLASH_ATTR decoder_alloc_should_reject_zero_byte_input_buffer(void) {
    ASSERT_EQ(NULL, heatshrink_decoder_alloc(0,
            HEATSHRINK_MIN_WINDOW_BITS, 4));
    PASS();
}

TEST ICACHE_FLASH_ATTR decoder_sink_should_reject_null_hsd_pointer(void) {
...
TEST ICACHE_FLASH_ATTR decoder_sink_should_reject_null_input_pointer(void) {
...
TEST ICACHE_FLASH_ATTR decoder_sink_should_reject_null_count_pointer(void) {
    uint8_t input[] = {0,1,2,3,4,5};
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256,
        HEATSHRINK_MIN_WINDOW_BITS, 4);
...
TEST ICACHE_FLASH_ATTR decoder_sink_should_reject_excessively_large_input(void) {
    uint8_t input[] = {0,1,2,3,4,5};
...
TEST ICACHE_FLASH_ATTR decoder_sink_should_sink_data_when_preconditions_hold(void) {
    uint8_t input[] = {0,1,2,3,4,5};
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256,
        HEATSHRINK_MIN_WINDOW_BITS, 4);
    size_t count = 0;
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_return_empty_if_empty(void) {
    uint8_t output[256];
    size_t out_sz = 0;
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256,
        HEATSHRINK_MIN_WINDOW_BITS, 4);
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_reject_null_hsd(void) {
    uint8_t output[256];
    size_t out_sz = 0;
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_reject_null_output_buffer(void) {
    size_t out_sz = 0;
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256,
        HEATSHRINK_MIN_WINDOW_BITS, 4);
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_reject_null_output_size_pointer(void) {
    uint8_t output[256];
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256,
        HEATSHRINK_MIN_WINDOW_BITS, 4);
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_expand_short_literal(void) {
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_expand_short_literal_and_backref(void) {
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_expand_short_self_overlapping_backref(void) {
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_suspend_if_out_of_space_in_output_buffer_during_literal_expansion(void) {
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_suspend_if_out_of_space_in_output_buffer_during_backref_expansion(void) {
    uint8_t input[] = {0xb3, 0x5b, 0xed, 0xe0, 0x40, 0x80}; //"foofoo"
    uint8_t output[4];
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256, 7, 7);
    memset(output, 0, sizeof(*output));
...
TEST ICACHE_FLASH_ATTR decoder_poll_should_expand_short_literal_and_backref_when_fed_input_byte_by_byte(void) {
    uint8_t input[] = {0xb3, 0x5b, 0xed, 0xe0, 0x40, 0x80}; //"foofoo"
    uint8_t output[7];
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256, 7, 7);
    memset(output, 0, sizeof(*output));
...
TEST ICACHE_FLASH_ATTR decoder_finish_should_reject_null_input(void) {
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256, 7, 7);
...
TEST ICACHE_FLASH_ATTR decoder_finish_should_note_when_done(void) {
    uint8_t input[] = {0xb3, 0x5b, 0xed, 0xe0, 0x40, 0x80}; //"foofoo"
...
TEST ICACHE_FLASH_ATTR gen(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 7);
...
TEST ICACHE_FLASH_ATTR  decoder_should_not_get_stuck_with_finish_yielding_MORE_but_0_bytes_output_from_poll(void) {
    uint8_t input[512];
...
static int ICACHE_FLASH_ATTR compress_and_expand_and_check(uint8_t *input, uint32_t input_size, cfg_info *cfg) {
...
TEST ICACHE_FLASH_ATTR data_without_duplication_should_match(void) {
...
TEST ICACHE_FLASH_ATTR data_with_simple_repetition_should_compress_and_decompress_properly(void) {
...
TEST ICACHE_FLASH_ATTR data_without_duplication_should_match_with_absurdly_tiny_buffers(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 3);
...
TEST ICACHE_FLASH_ATTR data_with_simple_repetition_should_match_with_absurdly_tiny_buffers(void) {
    heatshrink_encoder *hse = heatshrink_encoder_alloc(8, 3);
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256, 8, 3);
    uint8_t input[] = {'a', 'b', 'c', 'a', 'b', 'c', 'd', 'a', 'b',
                       'c', 'd', 'e', 'a', 'b', 'c', 'd', 'e', 'f',
...
static void ICACHE_FLASH_ATTR fill_with_pseudorandom_letters(uint8_t *buf, uint32_t size, uint32_t seed) {
...
TEST ICACHE_FLASH_ATTR pseudorandom_data_should_match(uint32_t size, uint32_t seed, cfg_info *cfg) {
    uint8_t input[size];
    if (cfg->log_lvl > 0) {
...
TEST ICACHE_FLASH_ATTR small_input_buffer_should_not_impact_decoder_correctness(void) {
    int size = 5;
    uint8_t input[size];
    cfg_info cfg;
    cfg.log_lvl = 0;
...
TEST ICACHE_FLASH_ATTR regression_backreference_counters_should_not_roll_over(void) {
...
TEST ICACHE_FLASH_ATTR regression_index_fail(void) {
...
TEST ICACHE_FLASH_ATTR sixty_four_k(void) {
