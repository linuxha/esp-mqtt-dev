...
static void ICACHE_FLASH_ATTR usage(void) {
...
static ICACHE_FLASH_ATTR  void die(char *msg) {
    fprintf(stderr, "%s\n", msg);
    exit(EXIT_FAILURE);
}

static void ICACHE_FLASH_ATTR report(config *cfg);

/* Open an IO handle. Returns NULL on error. */
static io_handle ICACHE_FLASH_ATTR *handle_open(char *fname, IO_mode m, size_t buf_sz) {
    io_handle *io = NULL;
...
static ssize_t ICACHE_FLASH_ATTR handle_read(io_handle *io, size_t size, uint8_t **buf) {
    LOG("@ read %zd\n", size);
...
static int ICACHE_FLASH_ATTR handle_drop(io_handle *io, size_t size) {
    LOG("@ drop %zd\n", size);
...
static ssize_t ICACHE_FLASH_ATTR handle_sink(io_handle *io, size_t size, uint8_t *input) {
    LOG("@ sink %zd\n", size);
    ...
    os_memcpy(&io->buf[io->fill], input, size);
    io->fill += size;
    return size;
}

static voidICACHE_FLASH_ATTR handle_close(io_handle *io) {
    if (io->fd != -1) {
        if (io->mode == IO_WRITE) {
            ssize_t written = write(io->fd, io->buf, io->fill);
            io->total += written;
...
static void ICACHE_FLASH_ATTR close_and_report(config *cfg) {
    handle_close(cfg->in);
...
static int ICACHE_FLASH_ATTR encoder_sink_read(config *cfg, heatshrink_encoder *hse,
...
static int ICACHE_FLASH_ATTR encode(config *cfg) {
...
static int ICACHE_FLASH_ATTR decoder_sink_read(config *cfg, heatshrink_decoder *hsd,
        uint8_t *data, size_t data_sz) {
    io_handle *out = cfg->out;
    size_t sink_sz = 0;
    size_t poll_sz = 0;
...
static int ICACHE_FLASH_ATTR decode(config *cfg) {
    uint8_t window_sz2 = cfg->window_sz2;
    size_t window_sz = 1 << window_sz2;
...
static void ICACHE_FLASH_ATTR proc_args(config *cfg, int argc, char **argv) {
