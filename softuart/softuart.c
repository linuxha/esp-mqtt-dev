...
void ICACHE_FLASH_ATTR Softuart_Init(Softuart *s, uint16_t baudrate)
{
	//disable rs485
	s->is_rs485 = 0;
	...
	if(! _Softuart_Instances_Count) {
		...
		//gpio_init(); // Removed as this is done elsewhere
	}
...
void ICACHE_FLASH_ATTR Softuart_Writeline_Nextion(Softuart *s, const char *c )
{
	while ( *c ) {
      	Softuart_Putchar(s,( u8 )*c++);
	}
	Softuart_Putchar(s,( u8 )0xff);
	Softuart_Putchar(s,( u8 )0xff);
	Softuart_Putchar(s,( u8 )0xff);
}

uint8_t ICACHE_FLASH_ATTR Softuart_Readline_Nextion(Softuart *s, char* Buffer, uint8_t MaxLen )
{
	uint8_t NextChar;
	uint8_t len = 0;
	char *tbuf;

	tbuf=Buffer;  // store in case we need to parse numbers 0x71
